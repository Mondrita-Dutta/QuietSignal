...
const src = resolve(ROOT, '..', 'backend', 'contracts', 'managed', 'quietsignal');
const uiPublic = resolve(ROOT, 'public');

mkdirSync(resolve(uiPublic, 'managed', 'quietsignal', 'keys'), { recursive: true });
mkdirSync(resolve(uiPublic, 'zkir'), { recursive: true });

// Copy keys and zkir into public directory
if (existsSync(resolve(src, 'keys'))) {
  cpSync(resolve(src, 'keys'), resolve(uiPublic, 'managed', 'quietsignal', 'keys'), { recursive: true });
...
if (existsSync(resolve(src, 'zkir'))) {
  cpSync(resolve(src, 'zkir'), resolve(uiPublic, 'managed', 'quietsignal', 'zkir'), { recursive: true });
...
const srcContracts = resolve(ROOT, 'src', 'contracts', 'quietsignal');
