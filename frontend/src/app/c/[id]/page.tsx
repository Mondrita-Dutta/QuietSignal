...
export default function TopicSurvey() {
  ...
  const isExpired = topic?.endDate ? new Date(topic.endDate).getTime() < Date.now() : false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (topic?.questions && topic.questions.length > 0 && Object.keys(answers).length === 0) {
    ...
    try {
      if (!topic?.publicKey) {
         throw new Error("This topic is missing a public encryption key. Cannot securely encrypt payload.");
      }
      const pid = await generateProofAndSubmit(answers, topicId, topic.publicKey);
      setProofId(pid);
      setStage('SUCCESS');
    } catch (e) {
      ...
      <header className="shrink-0 z-50 bg-[var(--color-cotton-bg)]/80 backdrop-blur-md border-b border-slate-300 shadow-sm felt-texture sticky top-0">
        <nav className="flex flex-col sm:flex-row justify-between items-center w-full px-[var(--spacing-container-padding)] py-4 max-w-4xl mx-auto gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-headline-md font-bold tracking-tight text-slate-900 drop-shadow-sm">QUIETSIGNAL</h1>
          </div>
        ...
        <AnimatePresence mode="wait">
          {stage === 'IDLE' || stage === 'CONNECTING' ? (
            ...
            >
              ...
              <div>
                ...
                <p className="font-body-lg text-slate-700 font-medium max-w-md">
                  Connect a Midnight wallet to participate in <strong>{topic?.title}</strong>. 1A.M. Wallet is recommended for stability. Your identity remains private.
                </p>
              </div>
              <button 
                onClick={handleConnect}
            ...
            >
              ...
              <div>
                ...
                <p className="font-body-lg text-slate-700 font-medium max-w-md mx-auto">
                  This survey stopped accepting new responses on {new Date(topic!.endDate!).toLocaleDateString()}.
                </p>
              </div>
            </motion.div>
          ) : stage === 'FORM' ? (
            ...
            >
              <div className="flex flex-col gap-3 text-center mb-2">
                ...
                {topic?.bannerUrl && (
                  <div className="w-full h-48 md:h-64 rounded-[2rem] overflow-hidden mb-6 shadow-sm border border-white/40">
                    <img src={topic.bannerUrl} alt="Survey Banner" className="w-full h-full object-cover" />
                  </div>
                )}
              ...
              <button 
                type="submit" 
                disabled={topic?.questions && topic.questions.length > 0 ? Object.keys(answers).length === 0 : !answers['default']}
