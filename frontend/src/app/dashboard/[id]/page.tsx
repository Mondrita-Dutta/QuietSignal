...
export default function TopicDetails() {
  ...
  const fetchData = async () => {
    try {
      const [campRes, feedRes] = await Promise.all([
        ...
        fetch(`/api/signals?topicId=${topicId}`)
      ]);
      ...
      if (feedData.records) {
        ...
        const decryptedRecords = await Promise.all(feedData.records.map(async (r: any) => {
          if (r.encryptedAnswers && r.iv && r.encryptedAesKey && issuerPrivateKey) {
            const decrypted = await decryptSignal(r.encryptedAnswers, r.iv, r.encryptedAesKey, issuerPrivateKey);
            if (decrypted) {
               r.answers = decrypted;
            } else {
  ...
  const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}/c/${topicId}` : '';
  ...
  const downloadQR = () => {
    ...
    if (canvas) {
      ...
      downloadLink.download = `survey-qr-${topicId}.png`;
  ...
  if (!topic) return (
    <div className="min-h-screen bg-[var(--color-cotton-bg)] flex flex-col items-center justify-center text-center p-8 felt-texture">
      ...
      <p className="font-body-lg text-slate-700 mt-2 font-medium max-w-md">
        It might still be propagating through the network. If you just created this topic, please click the button below to force a refresh.
      </p>
      <button 
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-[var(--spacing-container-padding)] pt-12 w-full relative z-10 flex flex-col gap-10">
        ...
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/50 inset-puffy mb-6">
            ...
            <span className="font-label-sm text-slate-800 font-bold uppercase tracking-widest drop-shadow-sm">Live Survey • {topic.id}</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 bg-white rounded-[3rem] p-10 puffy-shadow felt-texture flex flex-col justify-center items-center text-center relative group overflow-hidden">
            ...
            <span className="font-headline-xl text-6xl text-slate-900 drop-shadow-md z-10">{signals.length}</span>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-8 px-2">
            <h2 className="font-headline-lg font-bold text-slate-900 drop-shadow-sm">Decrypted Signal</h2>
            <button 
          ...
          <div className="flex flex-col gap-6">
            {signals.length === 0 ? (
              <div className="bg-white/40 rounded-[3rem] p-16 text-center flex flex-col items-center inset-puffy border border-white/50">
                ...
                <h3 className="font-headline-md font-bold text-slate-800 drop-shadow-sm">No signal yet</h3>
                  ...
                  <div className="flex flex-col gap-6">
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-4">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="font-label-sm text-slate-500 font-bold tracking-widest uppercase">Response #{sortOrder === 'newest' ? signals.length - idx : idx + 1}</span>
