...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (walletAddress) {
      fetch(`/api/topics?wallet=${encodeURIComponent(walletAddress)}`)
        .then(r => r.json())
        .then(data => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setTopics(topics.filter(c => c.id !== id));
      } else {
  ...
  const displayedTopics = showAll ? sortedTopics : sortedTopics.slice(0, 4);
            ...
            <div className="felt-texture bg-[var(--color-cotton-pink)] rounded-[3rem] puffy-shadow p-8 sm:p-12 flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  placeholder="e.g. Q3 Employee Signal"
                  required
              ...
              {sortedTopics.length > 4 && (
                ...
                >
                  {showAll ? (
                    ...
                    <>View All {sortedTopics.length} Surveys <span className="material-symbols-outlined">expand_more</span></>
                  )}
                </button>
              )}
            </div>
            ...
            <div className="felt-texture bg-white rounded-[3rem] puffy-shadow p-8 sm:p-12 flex flex-col gap-8 border border-slate-200">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                 <div className="bg-[var(--color-cotton-bg)] rounded-3xl p-8 flex flex-col items-center justify-center border border-slate-200 inset-puffy text-center gap-2">
                   ...
                   <span className="text-4xl font-bold text-slate-900">{topics.length}</span>
                 ...
                 <div className="bg-[var(--color-cotton-bg)] rounded-3xl p-8 flex flex-col items-center justify-center border border-slate-200 inset-puffy text-center gap-2">
                   ...
                   <span className="text-4xl font-bold text-slate-900">{topics.reduce((acc, c) => acc + (c.responseCount || 0), 0)}</span>
               ...
               {topics.length > 0 && (
                 <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mt-6">
                   {(() => {
                     const barData = topics.map(c => ({ name: c.title.substring(0, 15) + (c.title.length > 15 ? '...' : ''), responses: c.responseCount || 0 }));
                     const allQuestions = topics.flatMap(c => c.questions || []);
                     const pieData = [
