...
export async function readDB(filename: string): Promise<any[]> {
  try {
    ...
    console.warn(`[QUIETSIGNAL DB] Vercel Blob read failed, falling back to local FS: ${err.message}`);
  }
...
export async function writeDB(filename: string, data: any[]): Promise<void> {
  ...
  try {
    ...
    console.warn(`[QUIETSIGNAL DB] Vercel Blob write failed, falling back to local FS: ${err.message}`);
  }
