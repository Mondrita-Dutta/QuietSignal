      ...
      switch (choice.trim()) {
        case '1': {
          ...
          try {
            ...
            const tx = await deployed.callTx.broadcastSignal(randomNullifier);
            console.log(`\n  ✅ Signal stored securely using nullifier: ${Buffer.from(randomNullifier).toString('hex').substring(0,8)}...`);
